public class HelloWorld {
	/** Le programme commence ici */
	public static void main(String[] args) {
		System.out.println("Hello World!");
		System.out.println("Hello World!1");
		System.out.println("Hello World!");
		System.out.println("Hello World!2");
		System.out.println("Hello World!2");
		System.out.println("Hello World!");
		System.out.println("Hello World!23");
		System.out.println("Hello World!2555");
		System.out.println("Hello World!2666");
		System.out.println("Hello World!277888");
		System.out.println("Hello World!27788833333333");

		int t = 2;
		int bonusAllowance = 500;
		System.out.println(t);

		// Déclarez la variable int ongoingAllowance
		int ongoingAllowance = 200;
		// Déclarez la variable int savings
		int savings = 3000;

		// TODO : Déclarez une variable int nommée bonusAllowance avec la valeur 500

		// Affiche le résultat
		System.out.println("You have b an ongoing allowance of " + ongoingAllowance + "\nYour savings amount is "
				+ savings + "\nYou have a bonus allowance of " + bonusAllowance);

		System.out.println("Hello World!27788833333333");
		System.out.println("test");
		System.out.println("test2");
		

	}
    
    public int sommeEntier(int num1, int num2 ) {
    	
    	return num1 + num2;	
    
    }
    
}